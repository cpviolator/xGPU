...
int main(int argc, char** argv) {
  ...
  // allocate the GPU X-engine memory, delclare before 'goto' to keep compiler happy.
  ComplexInput *array_h = NULL;
  XGPUContext context;
  unsigned int ompMatLength;
  Complex *cuda_matrix_h = NULL;
  ...
  ompMatLength = nfrequency * ((nstation+1)*(nstation/2)*npol*npol);
...
#if !defined(POWER_LOOP) && !defined(BENCHMARK)
  cuda_matrix_h = context.matrix_h;
  if(count > 1) {
