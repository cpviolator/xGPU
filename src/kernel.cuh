...
__global__ static void shared2x2(float4 *matrix_real, float4 *matrix_imag, const int Nstation, const int write, cudaTextureObject_t tex)
{
...
#pragma message "BUFFER_DEPTH 2 " XSTR(__CUDA_ARCH__)
#else 
#define BUFFER_DEPTH 4 // Kepler optimal setting
#pragma message "BUFFER_DEPTH 4 " XSTR(__CUDA_ARCH__)
#endif
...
#if BUFFER_DEPTH==2
  LOAD(0, 0, tex);
#warning "BUFFER_DEPTH 2 in shared2x2"
#elif BUFFER_DEPTH==4  
  LOAD(0, 0, tex);
  LOAD(1, 1, tex);
#warning "BUFFER_DEPTH 4 in shared2x2"
#endif
...
#if BUFFER_DEPTH==2
  TWO_BY_TWO_COMPUTE(0);
  LOAD(1, NTIME_PIPE-1, tex);
#elif BUFFER_DEPTH==4
  TWO_BY_TWO_COMPUTE(0);
  TWO_BY_TWO_COMPUTE(1);
  LOAD(2, NTIME_PIPE-2, tex);
  LOAD(3, NTIME_PIPE-1, tex);
#endif
...
__global__ static void shared2x2(int4 *matrix_real, int4 *matrix_imag, const int Nstation, const int write,  cudaTextureObject_t tex)
{

// Set the degree of shared memory buffering to use
#if __CUDA_ARCH__ < 300 
#define BUFFER_DEPTH 2 // Fermi optimal setting
#warning "BUFFER_DEPTH 2"
#else 
#define BUFFER_DEPTH 4 // Kepler optimal setting
#warning "BUFFER_DEPTH 4"
#endif
...
#if BUFFER_DEPTH==2
  TWO_BY_TWO_COMPUTE(0);
  LOAD(1, NTIME_PIPE/4-1, tex);
#elif BUFFER_DEPTH==4
  TWO_BY_TWO_COMPUTE(0);
  TWO_BY_TWO_COMPUTE(1);
  LOAD(2, NTIME_PIPE/4-2, tex);
  LOAD(3, NTIME_PIPE/4-1, tex);
#endif
